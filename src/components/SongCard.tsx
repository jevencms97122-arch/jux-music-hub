...
function SongCard({ song, size = 'md', isActive = false, isPlaying = false, onPlay }: SongCardProps) {
  return (
    <div className={`${size === 'sm' ? 'w-full' : 'w-36 sm:w-40 flex-shrink-0'} text-left group transition transform hover:-translate-y-0.5 duration-150`}>
      <button
        onClick={() => onPlay?.(song)}
        className="w-full"
        type="button"
      >
        <div className="relative aspect-square rounded-lg overflow-hidden mb-2 bg-secondary will-change-transform">
          <img
            ...
            decoding="async"
          />
          <div className="absolute inset-0 bg-background/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 p-2">
            <Play className="h-10 w-10 text-foreground fill-foreground flex-shrink-0" />
          </div>
          {isActive && isPlaying && (
